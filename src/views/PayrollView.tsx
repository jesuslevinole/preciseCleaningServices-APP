...
import { Search, Calendar, User, DollarSign, CheckCircle, Activity, MapPin } from 'lucide-react';
...
export default function PayrollView({ onOpenMenu }: PayrollViewProps) {
  ...
  const [selectedStatus, setSelectedStatus] = useState(''); // Filtro de Estatus
  ...
  // Cálculos dinámicos basados EN LOS REGISTROS FILTRADOS
          ...
          <tbody>
            {isLoading ? <tr><td colSpan={6} style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>Loading payroll data...</td></tr> : 
             ...
             filteredRecords.map(record => {
               ...
               return (
                 <tr key={record.id} style={{ transition: 'background-color 0.2s' }} onMouseEnter={e => e.currentTarget.style.backgroundColor = '#f8fafc'} onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}>
                   ...
                   <td style={{...s.td, textAlign: 'right'}}>
                     {isPaid ? (
                       <button onClick={() => handleMarkAsPending(record.id as string)} style={{ background: 'none', border: '1px solid #e2e8f0', color: '#64748b', padding: '6px 12px', borderRadius: '6px', fontWeight: 600, cursor: 'pointer', fontSize: '0.8rem' }}>Undo</button>
                     ) : (
                       <button onClick={() => handleMarkAsPaid(record.id as string)} style={{ backgroundColor: '#10b981', color: 'white', border: 'none', padding: '6px 12px', borderRadius: '6px', fontWeight: 600, cursor: 'pointer', fontSize: '0.8rem' }}>Mark Paid</button>
                     )}
                   </td>
